...
int opponent_attacks(int fd_attack, int fd_respond, int my_table[][GRID_SIZE], int *my_ships_sunk) {
    char buf[2] = {0};
    int read_size = read(fd_attack, buf, 2);
    ...
    int write_size = write(fd_respond, response, strlen(response) + 1);
    check_error(write_size > 0, "failed to write response to FIFO");

    return (*my_ships_sunk >= SHIP_COUNT) ? -1 : 0;  // -1 if player lost
}
...
while (ships_sunk < SHIP_COUNT && my_ships_sunk < SHIP_COUNT){
	int result;
	        ...
	        } while (opponent_result == 0 && my_ships_sunk < SHIP_COUNT);
